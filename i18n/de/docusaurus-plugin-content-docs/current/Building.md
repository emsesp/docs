...
Bevor Sie die Firmware erstellen, müssen Sie zunächst die WebUI-Distributionsdateien erstellen, indem Sie entweder `pio run -e build-webUI` aus dem Stammverzeichnis oder `pnpm build-webUI ` aus dem Ordner `interface` ausführen. Dadurch wird die Datei `src/ESP32React/WWWData.h` erstellt, die von der Firmware verwendet wird.
...
Die WebUI kann in Echtzeit entwickelt und getestet werden, indem Scheindaten aus der Datei `/mock-api/restServer.ts` verwendet werden. Dies ist nützlich, wenn Sie Änderungen an der WebUI vornehmen, Übersetzungen testen oder das Verhalten bei Änderungen der Testdaten beobachten.
