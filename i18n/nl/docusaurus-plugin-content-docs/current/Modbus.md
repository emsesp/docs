...
Met de Modbus-connector kunnen entiteiten worden gelezen en geschreven via het Modbus/TCP-protocol. Zie [Settings](Modbus-Settings.md) voor het inschakelen en configureren van de Modbus-connector.
...
EMS-ESP maakt een Modbus-server aan voor elk apparaat dat is aangesloten op de EMS-bus, inclusief EMS-ESP zelf. Elke server wordt geadresseerd met een numerieke identifier die het apparaattype weergeeft. Zie [Server Unit Identifiers](Modbus-Server-IDs.md) voor een lijst met alle mogelijke server-ID's.

Een server organiseert alle entiteiten van het apparaat dat hij vertegenwoordigt in blokken die overeenkomen met de entiteittags (bijvoorbeeld `hc1` voor het eerste verwarmingscircuit). De entiteiten worden geadresseerd met een offset binnen het registerblok. Zie [Register Blocks](Modbus-Register-Blocks.md) voor een lijst met alle blokoffsets.
...
Zie [Entity/Register Mapping](Modbus-Entity-Registers.md) voor een lijst van alle registeroffsets, gegevenstypen en schaalfactoren.
...
EMS-ESP kent een speciale unit identifier, `1`, toe aan een server die algemene informatie over EMS-ESP verschaft. De registers van de systeemserver zijn gedocumenteerd [here](Modbus-System-Server.md).
...
Na het toevoegen of wijzigen van EMS-entiteiten moet ook de registertoewijzing worden bijgewerkt. De Modbus-instellingen worden opgeslagen in het bestand `src/core/modbus_entity_parameters.hpp` en bevatten het tagtype, de registeroffset en het aantal registers voor elke entiteit.
...
Nadat het aangepaste `modbus_entity_parameters.hpp` bestand en de documentatie (.md bestand) zijn bijgewerkt, bouw je het, test je het en commit je het en push je het naar de EMS-ESP dev repository.
