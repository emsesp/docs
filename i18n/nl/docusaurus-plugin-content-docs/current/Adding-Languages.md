...
- Maak de map XY aan in `interface/src/i18n`
...
- Bewerk `src/system.cpp` regel 45 en voeg `EMSESP_LOCALE_XY` toe aan de `languages[]` array
- Bewerk `src/local_translations.h` en voeg de definities toe door `#define EMSESP_LOCALE_XY "xy"` toe te voegen
- voeg uw vertaling toe als `, "your text"` aan het einde van elke `MAKE_PSTR_LIST()` binnen het haakje het moet er ongeveer zo uitzien: `MAKE_PSTR_LIST(tag, "en", "de", "nl", "se", "pl", "xy")`
