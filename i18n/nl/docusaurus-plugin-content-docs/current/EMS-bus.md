...
Het circuit fungeert als een servicesleutel en gebruikt daarom een ID 0x0B. Deze ID is gereserveerd voor speciale apparaten die bedoeld zijn voor servicemonteurs.
...
Elk verbonden apparaat kan reageren op een Polling verzoek met een bevestiging door een enkele byte met zijn eigen ID terug te sturen. In ons geval zouden we luisteren naar een `[0x8B] <BRK>` (dat betekent wij) en dan `[0x0B] <BRK>` terugsturen om te zeggen dat we leven en klaar zijn.

Polling is ook de trigger om te beginnen met het verzenden van pakketten die in de wachtrij staan om verzonden te worden. Dit moet binnen 200 ms gebeuren anders zal de busmaster een time-out krijgen.

### EMS Uitzending

Als een apparaat naar iedereen uitzendt, is er geen specifieke bestemming nodig. `[dest]` is altijd 0x00.

De onderstaande tabellen laten zien welke typen regelmatig worden uitgezonden door de ketel (in dit geval ID 0x08) en thermostaat (ID 0x17). De **datalengte** is exclusief de 4 bytes header en CRC en de **naam** verwijst naar die in de Duitse [EMS wiki](https://emswiki.thefischer.net/doku.php?id="wiki:ems:telegramme").
...
| Ketel (0x08) | 0x18 | UBAMonitorFast | keteltemps, vermogen, gas/pump schakelaars | 25 bytes | 10 seconden |
| Ketel (0x08) | 0x19 | UBAMonitorSlow | keteltemperatuur en timings | 22 bytes | 60 seconden |
...
| ----------------- | ------- | ----------------- | --------------------------------------------------- | ---------- |
| Thermostaat (0x17) | 0x06 | RCTime | geeft de tijd en datum op de thermostaat weer | 60 seconden |
| Thermostaat (0x17) | 0x91 | RC30StatusMelding | geeft huidige en ingestelde temperaturen | 60 seconden | weer
...
Bij het uitvoeren van een verzoek om gegevens te lezen is de `[src]` ons apparaat `(0x0B)` en moet de `[dest]` zijn MSB (8e bit) hebben ingesteld. Stel dat we gegevens van de thermostaat opvragen, dan gebruiken we `[dest] = 0x97` omdat RC20 een ID heeft van 0x17. In emsesp-logs wordt dit verzoek weergegeven met `R`` between`[src]`and`[dst]`: _"Ik(0x0B) R Thermostaat(0x17)..."_.

Na een schrijfopdracht heeft de `[dest]` niet de 8e bit ingesteld en na deze schrijfopdracht verzendt het bestemmingsapparaat een enkele byte 0x01 voor succes of 0x04 voor mislukking. In emsesp-logs wordt dit schrijven weergegeven met `W` tussen `[src]` en `[dst]`: _"Me(0x0B) W Thermostaat(0x17)..."_.
...
In `system/info` staan de apparaten met de handlers (type-ids die worden verwerkt):

- `handlers received`: Telegrammen die vaak door het apparaat naar iedereen worden uitgezonden: _"Ketel(0x08) B Alle(0x00)..."_
- `handlers fetched`: Telegrammen die niet worden uitgezonden en één keer per minuut door EMS-ESP worden opgevraagd: _"Me(0x0B) R Ketel(0x08) .. Ketel(0x08) W Me(0x0B)..."_
- `handlers pending`: Telegrammen nog niet ontvangen of leeg bij ophalen. Voorbeeld: ems-ketels gebruikt telegram 0x18 om actuele waarden te controleren, ems+ketels gebruikt telegram 0xE4 voor dezelfde informatie. Als u 0x18 in de ontvangen lijst aantreft, hebt u een ems-ketel en is 0xE4 in afwachting.
