...
- stel `thermostat/hc<x>/control` (of Room Control in de WebUI) in op de thermostaat op afstand die je wilt bedienen
- stuur `thermostat/hc<x>/remotetemp` met de gewenste temperatuur. Optioneel voor een RC100H stuur je `thermostat/hc<x>/remotehum` met een waarde voor de luchtvochtigheid
...
- `thermostat/hc<x>/remotetemp` verzenden met een waarde van `-1`
- zet `thermostat/hc<x>/control` terug naar het type hoofdthermostaatmodel
...
:::warning Disclaimer Met het op afstand instellen van de luchtvochtigheid kun je een warmtepomp instellen om te gaan koelen, zelfs als het onder het werkelijke dauwpunt is, wat condensatie op leidingen en de vloer kan veroorzaken en vervolgens het systeem kan beschadigen. Gebruik op eigen risico!
:::
...
Als je een externe Dallas-temperatuursensor op de EMS-ESP hebt aangesloten, kun je de `value` op deze waarde instellen door `temperaturesensor/<dallas_id_or_name>` als `value` te gebruiken.
...
Je kunt het systeem-API eindpunt `message` gebruiken om een tekstbericht naar het logboek en MQTT te sturen, maar het bericht kan ook complexe logica bevatten zoals in de Scheduler. Bijvoorbeeld het verzenden van
