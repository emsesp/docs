...
De standaard 'fabrieksconfiguratie' start een WiFi Access Point genaamd `ems-esp`. Maak hier verbinding mee met het WPA-wachtwoord `ems-esp-neo`. Wanneer u wordt gevraagd om in te loggen (captive portal), meldt u zich aan met de gebruikersnaam `admin` en het wachtwoord `admin`. Deze kunnen later worden gewijzigd. Als u een Ethernetkaart hebt, kunt u rechtstreeks verbinding maken met het IP-adres of via `http://ems-esp` of `http://ems-esp.local`.
...
- **Analoge sensoren inschakelen**. Hierdoor kan elke GPIO signalen verzamelen, of het nu gaat om een digitale I/O, een pulsteller of een ADC die mv meet. Er zijn nog veel meer opties beschikbaar. Merk op dat de `Factor` een deler is.
...
- **Booleaans formaat API/MQTT**. Dit is hoe booleaanse waarden worden geschreven in de MQTT payloads en API JSON uitvoer.
- **Enum Formaat API/MQTT**. Dit is hoe lijstwaarden worden gepresenteerd in de MQTT payloads en API JSON, ofwel door de waarde of de indexpositie binnen de lijst. Als u Home Assitant gebruikt, ziet u niet de waarden maar gehele getallen voor sommige entiteiten, bijvoorbeeld in plaats van `off, hot, cold` wordt `0, 1, 2` weergegeven.
...
Als u `Custom` kiest, ziet u de volgende extra instellingen om de GPIO's handmatig te configureren voor uw eigen kaart. U kunt ook op elk moment `Custom` kiezen om de huidige instellingen van het boardprofiel weer te geven, zonder op te slaan.
...
- **EMS Tx-modus**. Tx Mode is de modus waarin EMS-ESP telegrammen verstuurt over de EMS-bus. Kies de modus die het beste werkt voor uw systeem en let op Tx-fouten in het Web Dashboard en `show ems` in de Console. Het wijzigen van de waarde heeft onmiddellijk effect.
  ...
  - `Hardware` gebruikt de interne hardware van de ESP om het telegram te verzenden. Telegrammen worden onmiddellijk verzonden. Het is de snelste en meest efficiënte methode, maar werkt alleen op sommige systemen.
- **EMS Bus ID**. De EMS-ESP kan meerdere apparaten simuleren. Houd de standaard `Service Key (0x0B)` aan, tenzij u meer dan één EMS gateways/interface kaart gebruikt. Het is belangrijk om te weten dat de Service Key ook wordt gebruikt door gecertificeerde service/maintenance technici bij het onderhoud van uw verwarmingssystemen, dus zorg ervoor dat u de EMS-ESP uitschakelt voordat ze arriveren, anders kunnen ze geen verbinding maken met de EMS-bus.
...
- **Developer Mode** schakelt geavanceerde functies in de WebUI in, zoals de opdracht Lezen van het systeemlogboek.
...
CORS, indien ingeschakeld, voegt nieuwe HTTP-headers toe aan elk webverzoek zodat de Web API `fetch` en `XMLHttpRequest` verzoeken kan doen op verschillende domeinen. Hiermee wordt de controle vooraf, die standaard hetzelfde oorsprongsbeleid volgt, uitgeschakeld. Zie [CORS](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS) voor meer details. Er is ook een browser addon "CORS unblock" die op dezelfde manier werkt.
...
- **Basis**. Alle onderwerpen worden voorafgegaan door de `Base` dus dit is belangrijk. Zorg ervoor dat deze uniek is als je meer dan één EMS-ESP met dezelfde broker gebruikt.
...
- **Opmaak**. De optie `Nested` groepeert alle apparaatgegevens in een enkel MQTT-onderwerp door ingesloten JSON-objecten te gebruiken, zoals `dhw` in `boiler_data`, `hc1` in `thermostat_data` enz. `As individual topics` zal dit opsplitsen in afzonderlijke MQTT-onderwerpen zonder groepering, dus MQTT-onderwerpen worden `boiler_data` en `boiler_data_dhw`, `thermostat_data` en `thermostat_data_hc1` enzovoort. Hetzelfde geldt voor de Analoge en Temperatuursensoren. De standaardinstelling is genest.
...
- **Enable MQTT Discovery** schakelt de integratie in met behulp van MQTT Discovery, volledig ondersteund met Home Assistant en gedeeltelijk met Domoticz.
  - **Ontdekkingstype**. Kies tussen "Home Assistant" en twee protocollen voor "Domoticz". `Domoticz (latest)` zal Cijfers, Schakelaars, Selecteren en andere moderne types gebruiken, terwijl `Domoticz` alleen Sensoren zal gebruiken voor alles en ook alle templating voorwaarden zal verwijderen.
  - **Entity ID-formaat**: Er zijn 3 opties. De eerste `single instance, long names` gebruikt het oudere < v3.5 formaat. De standaard en aanbevolen instelling voor alle nieuwe installaties is de 2e optie genaamd `Single instance, short name` die de EMS-ESP-apparaat entiteitnaam gebruikt, die vast is en kan worden gezien in de `Settings->Customization` pagina. De laatste optie moet worden gebruikt als er meer dan één versie van de EMS-ESP firmware wordt gebruikt, omdat het elke versie uniek maakt door alle MQTT topics vooraf te laten gaan door de basisnaam.
...
Op de pagina Gebruikers kunt u extra gebruikers aanmaken voor de captive portal en Telnet CLI. De standaardgebruiker is `admin` met wachtwoord `admin`.
...
Externe sensoren, zoals temperatuur en analoge sensoren, kunnen worden aangesloten op een reeks GPIO-pinnen op de ESP32-chip. Als je een BBQKees Gateway-bord gebruikt, heeft deze al een externe plug voor Dallas-temperatuursensoren die zonder extra configuratie zichtbaar zijn in de WebUI.

Om analoge sensoren toe te voegen klikt u op `Add` en kiest u tussen een normale Digitale in/out, een Teller (die on/off pulsen telt), ADC voor het meten van spanningen, Timer, Snelheid en PWM 0-2. Let op, de waarde van de teller wordt niet gereset bij opnieuw opstarten. Let op, de tellerwaarde blijft behouden en wordt niet gereset bij opnieuw opstarten.
...
- Puls is een digitale uitgang voor een enkele puls met instelbare lengte. Ingesteld op `on` door commando, gaat de sensor naar `off` nadat de puls is voltooid.
...
Tellers worden opgeslagen op reboot/update en elk uur naar nvs. In geval van crashes, stroomstoringen kan de waarde daarna lager zijn.

### Relais besturen

Een veel voorkomende toepassing is het aansturen van een relais om on/off een apparaat te schakelen. Dit kan worden gedaan door een digitale uitgangssensor toe te voegen en vervolgens een commando te maken om het te besturen. Het commando kan worden toegevoegd aan het dashboard en worden ingepland om on/off op bepaalde tijden te schakelen.
...
De pagina Aanpassingen toont alle geregistreerde entiteiten en biedt de mogelijkheid om opdrachten en waarden uit te sluiten van publicatie via MQTT/API of ze te verwijderen uit WebUI-pagina's. De Apparaten en het Dashboard tonen alleen entiteiten met een waarde, terwijl de module Aanpassing ze allemaal toont. Als een entiteit geen waarde heeft, wordt deze ondersteund door EMS-ESP, maar niet door uw boiler/thermostat/etc en wordt deze niet gepubliceerd of zichtbaar voor integraties zoals Home Assistant.
...
- voer elke week een periodieke systeemherstart uit (hoewel dit niet nodig zou moeten zijn!) met `system/restart` in het opdrachtveld
- stuur een bericht naar het logboek en mqtt met het commando `system/message`
...
De Scheduler, geïntroduceerd in **versie 3.7**, is uitgebreid met ondersteuning voor voorwaardelijke verklaringen en formules die kunnen worden gebruikt in de velden `Command` and/or `Value` voor Timer-, On Change- en Condition-typen.
...
- een voorwaarde moet een logische waarde `0` of `1` zijn, de voorwaarde is `true` alleen voor `1`, een rekenkundig resultaat `1` wordt ook geïnterpreteerd als `true` schemaopdracht uitgevoerd voor `3 > 2, 3 - 2` schemaopdracht niet uitgevoerd voor `3 < 2`, `3 + 2`
...
- De Bool-indeling (`0/1`, `off/on`, `OFF/ON` of `false/true`) en Enum-indeling (value/index) van de gebruiker worden gebruikt bij het evalueren van bool en enums. Controleer de juiste waarde voordat u een schema instelt door de API rechtstreeks te raadplegen. Ga bijvoorbeeld naar `http://ems-esp.local/api/thermostat` om te zien of het gebouw "medium" is en maak dan de regel met `thermostat/building == medium`.
...
- commando's gevolgd door een verdeler (`/`) moeten tussen haakjes worden gezet, bijvoorbeeld `(boiler/seltemp)/2`
- voorwaarde commando wordt alleen uitgevoerd bij een verandering van de voorwaarde van `false` naar `true`. Als de voorwaarde waar blijft, wordt het commando niet herhaald
...
- toegestane bewerkingen:
  ...
  - voorwaardelijke bewerkingen: `<cond1> ? <expr1> : <expr2>` Voorbeelden:
    - (toegestaan voor v3.7.2) `<cond> ? 5 + <expr1> : 5 + <expr2>`
    - (toegestaan sinds v3.7.2) `5 + (<cond> ? <expr1> : <expr2>)` en voorwaarden in cascade `<cond1> ? <cond2> ? <expr1> : <expr2> : <cond3> ? <expr3> : <expr4>`

Een On Change trigger is een lijst van entiteiten met het formaat `<device>/<entity>`, bijvoorbeeld `boiler/outdoortemp custom/setpoint`. Omdat entiteiten nooit tegelijkertijd veranderen, zijn logische bewerkingen zoals `&&` hier niet nuttig. Let op, het gebruik van `system` als de `<device>` wordt niet ondersteund.
...
- stel een waarde in met POST: Commando: `{"url":"http://server.tld/path/file", "header":{"content-type":"text/plain", "token":"mytoken"}` Waarde: het postbericht, als het een json is, is de content-type header ingesteld in de header, deze hoef je niet in te stellen.
...
Met webcommando's kan een service als [pushover](https://pushover.net) worden gebruikt om een push-bericht over gebeurtenissen te verzenden. Om een ander bericht te verzenden, maakt u een aangepaste entiteit in RAM met de naam `message`, of wat u maar wilt. Maak een schema On Change dat de verandering van dit bericht triggert en het pushbericht verzendt.

![grafik](https://github.com/user-attachments/assets/570576b5-b382-4ab2-bff3-4468291334a3)

Nu kunt u andere schema's maken met het commando `custom/message` en individuele tekst als gegevens gebruiken.
...
Een moderne warmtepomp kan bijvoorbeeld nieuwe functies hebben die niet zijn opgenomen in EMS-ESP. Hier zou u het `watch` commando kunnen gebruiken om het inkomende EMS-verkeer te bekijken in combinatie met het handmatig aanpassen van specifieke parameters en wanneer u het specifieke telegram en de offset hebt gevonden, maakt u een aangepaste entiteit om het type nauwkeurig af te stellen en de waarde te verifiëren. Vraag vervolgens of het wordt opgenomen in de volgende EMS-ESP release update.
