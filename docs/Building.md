...
| spiffs     | data | spiffs   | 0xAA0000      | 0x200000 (2 MB)    | for LittleFS/EMS-ESP filesystem | (optional contents of data folder) |
...
- With all the board/chip types, the `boot_app0.bin` and `partitions.bin` are typically the same file for each board. Only the `bootloader.bin` is different.
- See [EMS-ESP-Flasher-CLI](https://github.com/emsesp/EMS-ESP-Flasher-CLI/blob/main/README.md) for more information on the flasher CLI tool.
- See also https://docs.emsesp.org/Installing#manual-flashing-the-firmware for manual flashing of the firmware and the location of the .bin files.
