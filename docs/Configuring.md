...
- **EMS Bus ID**. The EMS-ESP can simulate multiple devices. Keep to the default `Gateway 2 (0x49)` unless using more than one EMS gateways/interface board or have conflicts with other ems-devices. It's important to note the Service Key (0x0B), used as default up to ems-esp 3.8.1, will also be used by certified service/maintenance engineers when servicing your heating systems so make sure you power-down EMS-ESP or change bus ID before they arrive or they will not be able to connect to the EMS bus.
...
- **Enable MQTT Discovery** enables the integration using MQTT Discovery, fully supported with Home Assistant and partially with Domoticz.
  ...
  - **Input Number Format**: (HA only) Select box or slider mode. Sliders will only be used for number entities with a range less than 100.
