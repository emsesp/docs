...
3. Download these 3 binary files associated with your board profile:
   ...
   - `s3_16M_P`
     ...
     - [boot_app0.bin](/bin/boot_app0.bin)
4. Install [esptool](https://docs.espressif.com/projects/esptool/en/latest/esp32/). You can download OS specific binaries from [here](https://github.com/espressif/esptool/releases) or use Python like `pip install esptool`.
5. Connect the ESP32/Gateway board to your computer via the USB port.
6. Open a terminal and navigate to the directory where the .bin files are downloaded.
7. Run the following command to flash the firmware depending on your board type:
